...
bool RedguardsRtxDatabase::readFile(const QString& filePath)
{
  QFile file(filePath);
  ...
  while (!file.atEnd()) {
    ...
    // Read total length (4 bytes, BIG ENDIAN) - not needed, but must read to advance
    // position
    ...
    int hasAudioFlag =
        ((unsigned char)hasAudioBytes[0] << 8) | (unsigned char)hasAudioBytes[1];
    ...
    if (hasAudio) {
      ...
      if (file.read(doubleSizeBytes.data(), 4) != 4)
        break;
      int doubleSize = 0;
      for (int i = 0; i < 4; ++i) {
...
bool RedguardsRtxDatabase::writeFile(const QString& filePath) const
{
  QFile file(filePath);
...
bool RedguardsRtxDatabase::applyChanges(const QString& changesFilePath)
{
...
RedguardsRtxEntry* RedguardsRtxDatabase::getEntry(const QString& label)
{
  if (mEntries.contains(label)) {
...
int RedguardsRtxDatabase::readLittleEndianInt(const QByteArray& data, int offset) const
{
...
int RedguardsRtxDatabase::readLittleEndianShort(const QByteArray& data, int offset) const
{
...
void RedguardsRtxDatabase::writeLittleEndianShort(QByteArray& data, int offset,
                                                   short value)
{
  if (offset + 2 > data.size()) {
...
QString RedguardsRtxDatabase::readString(const QByteArray& data, int offset,
                                         int length) const
{
  if (offset + length > data.size()) {
    return "";
  }
  // Use Latin1 encoding per Java implementation
  return QString::fromLatin1(data.mid(offset, length));
}

QByteArray RedguardsRtxDatabase::writeString(const QString& str)
{
  // Use Latin1 encoding per Java implementation
