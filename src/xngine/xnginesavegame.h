#ifndef XNGINESAVEGAME_H
#define XNGINESAVEGAME_H
...
class XngineSaveGame : public MOBase::ISaveGame
{
public:
  XngineSaveGame(QString const& file, GameXngine const* game,
                 bool const lightEnabled = false, bool const mediumEnabled = false);

  virtual ~XngineSaveGame();
...
protected:
  ...
  // Fetch the field - implemented as empty stub for XnGine
  virtual std::unique_ptr<DataFields> fetchDataFields() const {
    return std::make_unique<DataFields>();
  }
};

#endif  // XNGINESAVEGAME_H
